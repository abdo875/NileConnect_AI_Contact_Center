...
def test_telegram_webhook_start_command(client):
    ...
    with patch("app.telephony.telegram.client.TelegramClient.send_message", return_value={"ok": True}):
        resp = client.post("/api/v1/telephony/telegram/webhook", json=payload)
...
def test_telegram_webhook_text_yes_flow(client):
    ...
    with patch("app.telephony.telegram.client.TelegramClient.send_message", return_value={"ok": True}):
        # /start
        client.post("/api/v1/telephony/telegram/webhook", json={
            "update_id": 5002,
            "message": {"message_id": 1, "chat": {"id": chat_id}, "from": {"id": chat_id}, "text": "/start"},
        })
        # Customer: YES
        resp = client.post("/api/v1/telephony/telegram/webhook", json={
            "update_id": 5003,
            "message": {
                "message_id": 2,
                "chat": {"id": chat_id},
    ...
    db = SessionLocal()
    customer = db.query(Customer).filter(Customer.phone == f"TG_{chat_id}").first()
    followup = db.query(AIFollowup).filter(AIFollowup.customer_id == customer.id).order_by(AIFollowup.created_at.desc()).first()
...
def test_telegram_webhook_voice_flow(client):
    chat_id = 66604
    with patch("app.telephony.telegram.client.TelegramClient.get_file", return_value={"result": {"file_path": "voice/test.oga"}}), \
         patch("app.telephony.telegram.client.TelegramClient.download_file", return_value=b"fake_audio"), \
         patch("app.telephony.telegram.client.TelegramClient.send_voice", return_value={"ok": True}) as mock_voice, \
         patch("app.telephony.stt.stt_service.STTService.transcribe_audio", return_value="تمام المشكلة اتحلت"):
